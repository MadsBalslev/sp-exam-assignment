...
# Python script used to plot the benchmark results.csv.
...
def plot_data(df, show_sync):
	...
	os_managed = df[(df['Thread count'] == -1) & (df['Benchmark'].str.contains('Parallel'))]
	multithreaded = df[(df['Thread count'] > 0) & (df['Benchmark'].str.contains('Parallel'))]
